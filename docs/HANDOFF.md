...
- [ ] **#8–12 inventory dashboard** — NEXT (frontend phase). Needs a **web client** — framework decision pending (Vite SPA vs Next.js) since it also shapes the review surface #21-30 (deployable per-spec URLs) and ideation UI #17-20. #8 = component gallery (live preview via lighter-example `<SpecView>`), #9 props table, #10 token inventory, #11 health panel, #12 usage.
  - ⚠️ **Still pending**: lighter-example emit `dist/catalog.json` (has tokens.json only) so ingestion runs against the LIVE example, not just the fixture — do with #36.
- [ ] #13–16 spec model+versioning
